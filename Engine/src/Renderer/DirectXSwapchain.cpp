...
namespace Engine
{
	DirectXSwapchain::DirectXSwapchain(const uint32_t pWidth, const uint32_t pHeight)
	{
		...
		swapchainInfo.SampleDesc.Quality = DirectXContext::Get()->m_4xMsaaState
	...
	void DirectXSwapchain::Resize(uint32_t pWidth, uint32_t pHeight)
	{
		...
		THROW_IF_FAILED(
			DirectXContext::Get()->m_CommandObject->ResetList(DirectXContext::Get()->m_CommandObject->
				GetCommandAllocator()));
		...
		for (UINT i = 0; i < k_SwapChainBufferCount; i++)
		{
			...
			DirectXContext::Get()->m_Device->
			                       CreateRenderTargetView(m_SwapchainBuffers[i].Get(), nullptr, rtvHeapHandle);
		...
		depthStencilDesc.SampleDesc.Quality = DirectXContext::Get()->m_4xMsaaState
			                                      ? (DirectXContext::Get()->m_4xMsaaQuality - 1)
			                                      : 0;
		...
		DirectXContext::Get()->m_Device->CreateDepthStencilView(m_DepthStencilBuffer.Get(), &dsvDesc,
		                                                        GetDepthStencilView());
